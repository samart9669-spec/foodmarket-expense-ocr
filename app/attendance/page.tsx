...
export default function AttendancePage() {
  ...
  const getMethodBadge = (method: string | null) => {
    ...
    return (
      <span className={`text-xs px-1.5 py-0.5 rounded ${colors[method] || colors.manual}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex items-center gap-3">
          ...
          />
          <button onClick={fetchRecords} className="btn-secondary text-sm">
            ค้นหา
          </button>
        </div>
      </div>
                ...
                >
                  ...
                  {employees.map((emp) => (
                    <option key={emp.id} value={emp.id}>
                      {emp.name} ({emp.employee_type === 'kitchen' ? 'ครัว' : 'ขาย'})
                    </option>
                  ))}
                </select>
              </div>
      ...
      <div className="card p-0 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400">
            ...
            <p>ไม่มีข้อมูลการเข้างานวันที่ {date}</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="table-header">ชั่วโมง</th>
                ...
                {records.map((rec) => (
                  <tr key={rec.id} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell">
                      <EmployeeTypeTag type={rec.employee_type} />
                    </td>
                    ...
                    <td className="table-cell">
                      {rec.regular_hours > 0 ? `${rec.regular_hours.toFixed(1)} ชม.` : '-'}
                    </td>
                    <td className="table-cell">
                      {rec.ot_hours > 0 ? (
                        <span className="text-orange-600 font-medium">
                          {rec.ot_hours.toFixed(1)} ชม.
                        </span>
                      ) : '-'}
                    </td>
                    <td className="table-cell text-gray-500 text-sm">
                      {rec.sales_point_name || '-'}
                    </td>
                    <td className="table-cell">
