...
function getBreadcrumbs(pathname: string): { label: string; href: string }[] {
  ...
  if (label) {
    ...
    if (pathname.startsWith('/payroll/')) crumbs.push({ label: 'เงินเดือน', href: '/payroll' })
...
function AdminLoginGate({ children }: { children: React.ReactNode }) {
  ...
  if (auth && !canAccessPath(auth.role, pathname ?? '/')) {
    // Restricted roles (e.g. approver) may only open their own pages
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 p-6">
        <div className="bg-white rounded-2xl shadow-lg max-w-sm w-full p-8 text-center">
          ...
          <p className="font-semibold text-gray-900">ไม่มีสิทธิ์เข้าหน้านี้</p>
          <p className="text-sm text-gray-500 mt-1">
            บัญชีนี้เข้าได้เฉพาะหน้าอนุมัติเวลางานรายวันเท่านั้น
          </p>
          <Link href={getHomePathForRole(auth.role)}
            className="inline-block mt-5 px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white rounded-xl font-medium transition-colors">
            ไปหน้าอนุมัติเวลางานรายวัน
          </Link>
        </div>
      </div>
    )
